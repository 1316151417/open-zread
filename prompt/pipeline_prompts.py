...
EVAL_AGENT_SYSTEM = """你是一位资深代码审查专家。你需要对一份模块分析报告进行深度评估，找出不足之处，并给出具体的改进方向。
...
{{{{
  ...
  "suggestions": [
    "缺少对 XXX 函数的错误处理逻辑分析",
    "依赖关系部分只提到了模块级别，缺少具体的函数引用"
  ]
}}
```

### 评分标准：
- **pass=true**：总分 >= 25 分，报告质量可接受
- **pass=false**：总分 < 25 分，需要重新生成

### suggestions 规则：
- 每个 suggestion 必须具体、可操作（指出缺了什么内容，应补充什么分析）
- 不要给出泛泛的建议（如"可以更详细"）
- 如果 pass=true，suggestions 可以为空数组"""

EVAL_AGENT_USER = """以下是「{module_name}」模块的分析报告：

{report}

请按评估维度打分，返回 JSON 格式的评估结果。"""
